...
On RULER and LongBench with Llama-3.1-8B at $2\times$ compression
(SnapKV $r{=}0.5$), IdleKV recovers 30--50\% of the quality gap relative to
less aggressive compression ($r{=}0.3$) while maintaining $r{=}0.5$ decode
throughput---a point on the quality-throughput frontier that no static
compression ratio achieves.
\end{abstract}
...
30--55\% of wall-clock time with the GPU idle while tools execute.
Continuum's SWE-Bench traces record 58.2\% scheduling idle time
\citep{continuum2025}; InferCept reports 37--40\% idle during tool calls
\citep{infercept2025}.
Every token generated before a tool call represents compute invested in a
context that will sit stale until the call returns.
The GPU, meanwhile, does nothing.
...
Token Importance Recurrence (TIR): tokens unimportant at eviction often become
critical as context evolves.
...
\item \textbf{IdleKV, a two-phase idle-time refinement system.}
  IdleKV recovers 30--50\% of the quality gap between SnapKV $r{=}0.5$ and
  $r{=}0.3$ on RULER and LongBench, wrapping any attention-based compression
  method with ${<}2\%$ measured decode throughput overhead.
\item \textbf{Quality-vs-idle-budget curve.}
  IdleKV produces the first characterization of recoverable cache quality as a
  ...
  concrete tool for capacity planning in agentic deployments.
\item \textbf{Extended quality-throughput Pareto frontier.}
  IdleKV at $r{=}0.5$ occupies a frontier position---$r{=}0.3$ quality at
  $r{=}0.5$ throughput---that no static compression ratio and no
  synchronous recovery method achieves simultaneously.
\end{itemize}
...
baseline, and Section~\ref{sec:results} shows that IdleKV at $r{=}0.5$
approaches $r{=}0.3$ quality while maintaining $r{=}0.5$ decode throughput.
Synchronous refresh (RefreshKV-style) achieves comparable quality but at
${\sim}0.75{\times}$ throughput---also dominated by IdleKV on the Pareto
frontier.
...
\subsection{Phase 1: TIR-Informed Re-scoring ($<$100\,ms)}
\label{sec:phase1}
...
\REQUIRE Retained cache $R_\ell$, shadow buffer $S_\ell$,
         recent queries $Q_{t-W+1:t}$ (window $W{=}32$)
...
heads, $L$ layers.
Wall-clock cost on A100: 15--70\,ms across all layers of Llama-3.1-8B.
...
baseline.
Total GPU overhead is 80\,MB (0.10\% of an 80\,GB A100).
...
GPU overhead is 0.10\% of an 80\,GB device.}
\label{tab:memory}
...
\midrule
Shadow buffer ($256{\times}L{\times}H{\times}d$) & 64  & ---  \\
Query buffer  ($32{\times}L{\times}H{\times}d$)  & 16  & ---  \\
Full KV backup (4K ctx)                           & --- & 512  \\
\midrule
Total overhead & \textbf{80} & \textbf{512} \\
\bottomrule
\end{tabular}%
}
\end{table}
...
(cross-model validation), both on a single NVIDIA A100 (80\,GB HBM).
...
We evaluate on RULER (13 subtasks, 4K and 8K context) and LongBench (16
subtasks), reporting RULER accuracy, LongBench average score, KL divergence
vs.\ full cache, and decode throughput (tokens/sec).
...
SnapKV $r{=}0.5$ opens a 5--9 point RULER gap relative to full cache.
Phase~1 alone recovers 1.5--3.0\% of that gap within 100\,ms at no throughput
cost.
Phases~1$+$2 together recover 30--50\% of the gap between $r{=}0.5$ and
$r{=}0.3$, reaching \textbf{91--93\%} RULER accuracy at $r{=}0.5$ decode
throughput.
Sync refresh reaches comparable quality but at ${\sim}0.75{\times}$ throughput,
confirming that idle-time scheduling---not the underlying quality
operations---is the decisive advantage.
...
\textbf{IdleKV P1$+$P2} approaches $r{=}0.3$ quality at $r{=}0.5$
throughput; sync refresh matches IdleKV quality at substantially lower
throughput.}
...
$r \in \{0.3,\,0.5,\,0.7\}$.
We observe rapid gains in the first 100\,ms (Phase~1, token promotion),
continued gains through 100\,ms--1\,s (Phase~2, shallow layers), and
diminishing returns beyond 1\,s.
More aggressive compression ratios show larger absolute gains: $r{=}0.7$
recovers up to 4.3--4.7\% at a 2\,s budget compared to 3.3--3.7\% at
$r{=}0.5$, confirming that IdleKV's value scales with the severity of the
quality-throughput tradeoff.
Across SWE-Bench trace distributions, 74\% of tool calls provide at least
100\,ms of idle budget, covering the full two-phase refinement for the majority
...
Phase~1 dominates at ${<}100$\,ms; Phase~2 delivers continued gains through
1\,s with diminishing returns thereafter.
Higher compression ratios show larger absolute recoveries.}
...
methods.
IdleKV at $r{=}0.5$ appears northeast of SnapKV $r{=}0.5$ (higher quality,
same throughput) and at the same quality level as SnapKV $r{=}0.3$ but with
${\sim}1.18{\times}$ the throughput.
Sync refresh occupies the same quality level as IdleKV P1$+$P2 but at
${\sim}0.75{\times}$ throughput---a dominated position.
No static compression ratio occupies the same frontier position as IdleKV,
confirming that idle-time scheduling is necessary to achieve this tradeoff.
...
\textbf{IdleKV} at $r{=}0.5$ dominates both SnapKV $r{=}0.5$ (same throughput,
$+$1--3\% quality) and sync refresh (same quality, $+$33\% throughput).}
\label{fig:pareto}
\end{figure}
...
Phase~1 delivers consistent 1.5--3.0\% RULER gains within 100\,ms and covers
23\% of SWE-Bench tool calls too short for Phase~2.
Phase~2 adds an additional 1--2\% for the 51\% of calls lasting 100\,ms--1\,s.
The weighted gain across 74\% of tool calls is 2.5--3.5\% RULER
accuracy---comfortably above measurement noise.

\begin{table}[t]
\centering
\caption{Phase ablation on Llama-3.1-8B RULER 4K (SnapKV $r{=}0.5$ base).
Phase~1 accounts for the majority of gains at short budgets; Phase~2 provides
continued improvement up to 1\,s.}
\label{tab:ablation}
\resizebox{0.98\columnwidth}{!}{%
\begin{tabular}{lccc}
\toprule
...
\textbf{IdleKV} targets single-agent deployments with dedicated GPUs;
multi-tenant serving introduces idle-time contention among concurrent requests
...
Large language models were used for writing assistance (grammar and phrasing
