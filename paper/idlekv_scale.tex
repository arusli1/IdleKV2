...
100\,ms at no throughput cost. Under the same calibration, Phases~1$+$2 could
recover a larger share of the gap between $r{=}0.5$ and $r{=}0.3$, but the
current reduced A10G scout has only validated the robustness of Phase~1; the
stability of longer-budget P1$+$P2 remains an explicit open question for the
next scout and the later scale-up matrix. Sync refresh is predicted to reach
comparable quality but at
