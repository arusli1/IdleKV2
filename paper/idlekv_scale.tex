...
Tables~\ref{tab:main_results}--\ref{tab:idle_budget} still contain calibrated
placeholders for the broader scale-up study, derived from three validated prior
measurements:
...
In the current measured scope, this short-idle Phase~1 behavior is the core
supported claim.
Longer-budget P1$+$P2 remains exploratory: the current reduced A10G scout has
only validated the robustness of Phase~1, and the stability of longer-budget
P1$+$P2 remains an explicit open question for the next scout and the later
scale-up matrix. Sync refresh is predicted to reach comparable quality but at
...
The short-idle Phase~1 result is the core supported claim in the current
measured scope.
The P1$+$P2 row remains exploratory placeholder data for the later scale-up
study; sync refresh is predicted to match IdleKV quality at substantially
lower throughput.}
...
+~P1$+$P2 (exploratory) & 91--93\% & 39--41 & $1.0\times$ \\
...
promotion).
For the current measured scope, that first-100\,ms region is the key region.
Continued gains through 100\,ms--1\,s (Phase~2, shallow layers) remain a
scale-up hypothesis rather than a validated core claim, and returns beyond
1\,s are likely diminishing even if Phase~2 later stabilizes.
...
Phase~1 is predicted to dominate at ${<}100$\,ms and is the main supported
claim in the current measured scope. Phase~2 may deliver continued gains
through 1\,s, but that longer-budget behavior remains a scale-up question
rather than a settled result here.
...
The current measured scope only establishes the narrower claim that Phase~1
moves the operating point northeast of the matching static compression
baseline.
If longer-budget P1$+$P2 later stabilizes, sync refresh is predicted to occupy
the same quality level but at ${\sim}0.75{\times}$ throughput---a dominated
position.
...
The current measured scope only requires IdleKV Phase~1 to improve quality at
the same decode operating point; any stronger P1$+$P2 dominance remains
exploratory.]\vspace{1.6cm}}}
\caption{Quality-throughput Pareto frontier (Llama-3.1-8B, RULER 4K).
\textbf{IdleKV Phase~1} at $r{=}0.5$ is predicted to improve on SnapKV
$r{=}0.5$ at the same decode operating point. Any stronger claim about
P1$+$P2 dominating sync refresh is deferred to the later scale-up study.}
\label{fig:pareto}
\end{figure}
...
lasting 100\,ms--1\,s.
In the current measured scope, only the Phase~1 part of this story should be
treated as durable until the longer-budget scout stabilizes.
...
Phase~1 is the core supported claim. Phase~2 remains exploratory until the
long-budget scout and larger-GPU matrix confirm that it is stable and worth
including in the main story.}
\label{tab:ablation}
\resizebox{0.98\columnwidth}{!}{%
\begin{tabular}{lccc}
\toprule
...
Broader claims about Phase~2, full LongBench integration, and the complete
