...
reference in the workshop-scale study.
...
study. In the single-A10G workshop study, the central question is whether
...
This version of the paper separates two evidence tiers.
The measured workshop-scale core is the delayed-query pilot plus the stable
single-A10G experiment matrix described above. Tables~\ref{tab:main_results}--
\ref{tab:idle_budget} still contain calibrated placeholders for the broader
scale-up study, derived from three validated prior measurements:
...
\citep{adakv2024}. As broader A100-scale runs complete, these ranges should be
replaced with measured point estimates and confidence intervals.

\subsection{Main Quality Results}

Table~\ref{tab:main_results} presents the target quality and throughput story
for the broader scale-up matrix on Llama-3.1-8B.
...
advantage once the scale-up baseline is fully measured.

\begin{table}[t]
\centering
\caption{Quality and throughput across the broader scale-up baselines (Llama-3.1-8B,
