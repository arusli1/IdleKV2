...
\begin{tikzpicture}[
    ...
    flow/.style={->, thick},
    aux/.style={->, semithick, dashed},
    label/.style={font=\scriptsize\bfseries, text=black!70}
  ]
  \node[main] (prefill) {Prefill prompt\\full forward pass};
  \node[main, right=of prefill] (compress) {Compress KV\\keep retained cache};
  \node[main, right=of compress] (decode) {Decode with\\compressed cache};
  \node[main, right=of decode] (tool) {Tool call\\idle window opens};

  \node[support, below=of compress] (fullkv) {CPU full-KV backup\\for prefill tokens};
  \node[support, below left=0.75cm and -0.10cm of decode] (query) {Query buffer\\recent layer-local states};
  \node[support, below right=0.75cm and -0.10cm of decode] (shadow) {Shadow buffer\\recently evicted KV};
  \node[refine, below=of tool] (phase1) {Phase 1\\re-score retained $+$ shadow\\target $\leq 100$\,ms};
  \node[refine, below=of phase1] (phase2) {Phase 2 (optional)\\progressive full-attention\\refresh from full KV};

  \draw[flow] (prefill) -- (compress);
  \draw[flow] (compress) -- (decode);
  \draw[flow] (decode) -- node[above, align=center] {tool emitted /\\generation paused} (tool);

  \draw[aux] (compress) -- (fullkv);
  \draw[aux] (decode) -- (query);
  \draw[aux] (decode) -- (shadow);

  \draw[flow] (tool) -- (phase1);
  \draw[aux] (query.east) -- ($(phase1.west)+(0.0,0.12)$);
  \draw[aux] (shadow.east) -- ($(phase1.west)+(0.0,-0.12)$);
  \draw[aux] (fullkv.east) -- ($(phase2.west)+(0.0,0.0)$);
  \draw[aux] (query.east) |- ($(phase2.west)+(0.0,0.15)$);
  \draw[flow] (phase1) -- node[right, align=center] {if time remains} (phase2);
  \draw[flow] (phase1.west) .. controls ($(phase1.west)+(-1.1,0)$) and ($(decode.south)+(0,-1.4)$) .. node[below, align=center] {swap refined cache\\resume decode} (decode.south);
  \draw[aux] (phase2.west) .. controls ($(phase2.west)+(-1.35,0)$) and ($(decode.south)+(0,-1.9)$) .. (decode.south);

  \node[label, above=0.20cm of $(prefill)!0.50!(decode)$] {Latency-sensitive path};
  \node[label, right=0.10cm of phase1.east, align=left] {Idle-time\\repair path};
\end{tikzpicture}%
}
\caption{IdleKV pipeline.  Compression runs during generation
(latency-sensitive path), while repair runs only during tool-call pauses.
Phase~1 uses recent query states and the shadow buffer to repair aggressive
compression quickly; Phase~2 optionally performs a slower refresh from the
CPU full-KV backup before the refined cache is swapped back into decode.}
...
\texttt{configs/scale\_core.yaml} finishes. The current evidence already supports the
...
\texttt{scripts/throughput\_spotcheck.py} script for this purpose.
...
That is the role of \texttt{configs/scale\_mechanism\_ablation.yaml}.
